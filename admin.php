...
if(isset($_POST["submit"])){
    ...
    $result = mysqli_query($conn, "SELECT * FROM admin_table WHERE email = '$email'");
    $row = mysqli_fetch_assoc($result);
    if(mysqli_num_rows($result)>0){
        if($password == $row["password"]){
            $_SESSION["loginform"] = true;
            $_SESSION["id"] = $row["id"];
            header("Location: addswat.php");
        }
        else{
...
.overlay {
	...
	background: linear-gradient(to right, #1a1d1f, #465052);
    ...
    <div class="container right-panel-active" id="container">
        <div class="form-container sign-in-container">
            <form class="" action="" method="post" autocomplete="off">
                ...
                <input type="email" placeholder="Email" required value="" id="email" name="email"/>
        ...
        <div class="overlay-container">
            <div class="overlay">
                <div class="overlay-panel overlay-left">
					...
					<br>
					<br>
					<button onclick="location.href='http://localhost/travel%20web/index1.php'">Back to Home page</button>
                </div>
            </div>
        </div>
    </div>
